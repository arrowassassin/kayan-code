...
The same template walks a 2-D grid. [Word Search-style problems]
(/problems/0079 doesn't exist in the bank — do LC 79 directly) mark the
current cell visited, try four neighbors, unmark on the way out:
...
1. LC 78 (Subsets) and LC 46 (Permutations) — on LeetCode itself; code both
   from memory until choose/explore/unchoose is muscle.
2. LC 39 (Combination Sum) — the start-index + sort-and-break combo.
...
4. LC 79 (Word Search) — grid backtracking with in-place marking.
5. [Path Sum III](/problems/0437-path-sum-iii) — the choose/unchoose
   discipline applied to a running hashmap on a tree.
6. LC 51 (N-Queens) — the capstone: constraint sets, pruning, and clean
